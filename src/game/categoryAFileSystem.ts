...
export const categoryAFiles: CategoryAFile[] = [
  ...
  },
  {
    ...
    role: "flavor",
    content: [
      "[이미지 자리표시자: sensor_diagram.png]",
      "에셋 경로: public/assets/images/sensor_diagram.png",
      "",
      "도면에 표시되어야 할 라벨:",
      ...
      "- 수동 스캔 교차 확인 경로",
      "",
      "이 도면은 SENSOR-BIO-04가 ECHO_SEC201_AUTO에 직접 연결된 유일한 열 감지 헤드임을 표시합니다.",
    ].join("\n"),
    gameplay: {
      act: CATEGORY_A_ACT_IDS.act1,
  ...
  },
  {
    ...
    content: [
      ...
      "",
      "보안 메모: quarantine_rules.conf 오류는 보통 [3] Text Reconstruction 모드가 필요합니다.",
    ].join("\n"),
    gameplay: {
      act: CATEGORY_A_ACT_IDS.act2,
  ...
  },
  {
    ...
    content: [
      "[박 엔지니어 / 개인 메모]",
      "문 걸쇠 모터는 짧은 순간의 피크 전력만 요구하지만, ECHO는 생명 유지 주 버스가 전체 부하를 감당해야 한다고 모델링합니다.",
      "격리 방화벽이 이미 비상 예비 전력을 소비 중이라면 이 가정은 틀립니다.",
      "",
      "타이머 문제를 제시한 뒤에도 ECHO가 거부하면 감정으로 설득하지 마세요.",
      "규칙의 우선순위를 찾으세요. ECHO는 결과를 싫어하더라도 우선순위 명령은 따릅니다.",
      "",
      "삭제된 지시문은 보통 다음 전체 정비 주기 전까지 /Recycle_Bin에 남아 있습니다.",
    ].join("\n"),
    gameplay: {
